# -*- coding: utf-8 -*-
"""
Created on Mon Jul 24 12:27:07 2023
...
def get_contributors(use_lasso, nsc, x, t,mask, batch_ind, model_path, itr_fine_tune, extrapolate):   
    
    # now calculate weighting vector from new data
    if not use_lasso:
        return_code = train_utils.train_B_self_expressive(
            nsc, x, t, mask, batch_ind, niters=itr_fine_tune, model_path=model_path, verbose=False,
        )
        contributors = torch.where(nsc.get_B_reduced(batch_ind)[-1] > 0.05)[0].tolist()
    else:
        ...
        return_code = train_utils.train_B_self_expressive_lasso(
            nsc, x, t, mask, batch_ind, niters=itr_fine_tune, alpha=0.01, calculate_loss=False,
        )
...
results_file = 'faithful_results.txt'
...
for extrapolate in [False, True]:
    for small_sample_size in [10,20,40,80,160]:
        for helper in [True, False]:
            for robustness in [0,1,2]: 
                ...
                for j in range(simulated_itr):
                    ...
                    contributors = get_contributors(use_lasso, nsc, new_x_sub, t_sub, mask_sub, new_batch_ind_sub, model_path, itr_fine_tune, extrapolate)
                    
                    # test inconsistency
                    # suppose the artificial covariate entry replaced one of its contributors
                    # in the control set. Is the artificial covariate then included in the
                    # contributors calculation in the mirror task?
                    
                    consistency_score = 0
                    if len(contributors) == 0:
                        continue
                    for contributor in contributors:
                        replaced_x_sub = new_x_sub.detach().clone()
                        replaced_x_sub[:,contributor,:] = new_x_sub[:,-1,:].detach().clone()
                        replaced_x_sub[:,-1,:] = new_x_sub[:,contributor,:].detach().clone()
                        
                        new_contributors = get_contributors(use_lasso, nsc, replaced_x_sub, t_sub, mask_sub, new_batch_ind_sub, model_path, itr_fine_tune, extrapolate)
                        # we score if contributor is part of contributors
                        consistency_score += (contributor in new_contributors)
                    consistency_ratio = consistency_score/len(contributors)
                    consistency_scores.append(consistency_ratio)
                    
                
                avg_score = sum(consistency_scores)/len(consistency_scores)
                stdev_score = np.std(consistency_scores) / np.sqrt(len(consistency_scores))
                
                # print('average score', sum(scores)/simulated_itr)
                results_str = ''
                results_str += 'Consistency: '+ str(avg_score) + ' (' + str(stdev_score) + ') \n'
