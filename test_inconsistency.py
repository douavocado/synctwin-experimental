...
#             if use_treated == True:
#                 model_id = 'train_treated-' + str(n_hidden) + '-' + str(iteration)
#             else:
#                 model_id = 'train_control-' + str(n_hidden) + '-' + str(iteration)
            
#             print('iteration', iteration)
#             print("Running simulation with seed {}, for n_hidden {} and use treated {}".format(seed, n_hidden, use_treated))
#             numpy.random.seed(random.randint(1,100))
...
#                 if regular:
#                     enc = SyncTwin.RegularEncoder(input_dim=2, hidden_dim=n_hidden)
#                     dec = SyncTwin.RegularDecoder(hidden_dim=enc.hidden_dim, output_dim=enc.input_dim, max_seq_len=train_step)
#                 else:
#                     enc = SyncTwin.GRUDEncoder(input_dim=2, hidden_dim=n_hidden)
...
#combs = combinations(list(range(20)), 2)
combs = [(i,j) for i in range(20) for j in range(20)]

diff = []
for comb in combs:
    print(comb)

    base_path_model1 = "models/{}-seed-".format(sim_id) + str(seed) + 'train_control-' + str(n_hidden)+'-' + str(comb[0])
    base_path_model2 = "models/{}-seed-".format(sim_id) + str(seed) + 'train_treated-' + str(n_hidden)+'-' + str(comb[1])
    ...
    _1,_,similarity = procrustes(C1,C2)
    
    print('procrustes normalised distance:', similarity)
    diff.append(similarity)

print('avg diff', sum(diff)/len(diff))
print('std', np.std(diff)/np.sqrt(len(diff)))